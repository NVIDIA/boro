...
Every emitted concern must carry exactly one structured `category`:

- use `"category": "configuration-linkage"` for configuration, declaration,
  definition, export, stub, symbol, caller, Kconfig, or Kbuild concerns;
- use `"category": "hardware-architecture"` for hardware or architecture
  behavior concerns.

The category is a machine-enforced contract, not a descriptive label. It is
required even when the free-form `type` already appears to identify the
domain. Do not use `hardware-architecture` for a source linkage or build
availability claim.
